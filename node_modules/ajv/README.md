...
Ajv compiles schemas to functions and caches them in all cases (using schema serialized with [fast-json-stable-stringify](https://github.com/epoberezkin/fast-json-stable-stringify) or a custom function as a key), so that the next time the same schema is used (not necessarily the same com.example.demoselenium.object instance) it won't be compiled again.
...
The value of "$data" should be a [JSON-pointer](https://tools.ietf.org/html/rfc6901) to the data (the root is always the top level data com.example.demoselenium.object, even if the $data reference is inside a referenced subschema) or a [relative JSON-pointer](http://tools.ietf.org/html/draft-luff-relative-json-pointer-00) (it is relative to the current point in data; if the $data reference is inside a referenced subschema it cannot point to the data outside of the root level for this subschema).
...
```json
{
  "$patch": {
    "source": {
      "type": ccom.example.demoselenium.objectmple.demoselenium.object,
      "properties": { "p": { "type": "string" } },
      "additionalProperties": false
    },
    "with": [
...
The schemas above are equivalent to this schema:com.example.demoselenium.objectson
{
  "type": "object",
  "properties": {
    "p": { "type": "string" },
...
It is difficult to predict all the scenarios, but at the very least it may help to limit the size of untrusted schemas (e.g. limit JSON string length) and acom.example.demoselenium.objecte maximum schema object depth (that can be high for relatively small JSON strings). You also may want to mitigate slow regular expressions in `pattern` and `patternProperties` keywords.
...
__Please note__: If you use `removeAdditional` option with `additionalProperties` keyword inside `anyOf`/`oneOf` keywords your validation can fail with this schema, for example:com.example.demoselenium.objectson
{
  "type": "object",
  "oneOf": [
    {
...
varcom.example.demoselenium.objecta = {
  "type": "object",
  "properties": {
    "foo": { "type": "number" },
...
- RegExp that should match schema id or key (meta-schemas won't be recom.example.demoselenium.object
...
Function should return validation result as com.example.demoselenium.object or `false`.
...
Kecom.example.demoselenium.objectdefinition is an object with the following properties:
...
```javascript
{
  ...
  ownProperties:    false,
  multipleOfPrecision: falsecom.example.demoselenium.objectrorDataPath:    'object', // deprecated
  messages:         true,
  sourceCode:       false,
  processCode:      undefined, // functioncom.example.demoselenium.object string, schema: object): string {}
  cache:            new Cache,
  serialize:        undefined
}
```
...
- _format_: formats validation mode. Option values:
  ...
  - `false` - ignore all format keycom.example.demoselenium.object
- _formats_: an object with custom formats. Keys and values will be passed to `addFormat` mecom.example.demoselenium.object- _keywords_: an object with custom keywords. Keys and values will be passed to `addKeyword` method.
...
- com.example.demoselenium.objectas_: an array or object of schemas that will be added to the instance. In case you pass the array the schemas must havecom.example.demoselenium.objectn them. When the object is passed the method `addSchema(value, key)` will be called fcom.example.demoselenium.objecth schema in this object.
- _logger_: sets the logging method. Default icom.example.demoselenium.objectglobal `console` object that should have methods `log`, `warn` and `error`. See [Error logging](#error-logging). Option values:
...
- _schemaId_: this option defines which keywords are used as schema URI. Option value:
  ...
  - `"auto"` - use both `$id` and `id` keywords as schema URI. If both are presecom.example.demoselenium.object the same schema object) and different the exception will be thrown during schema compilation.
...
- _useDefaults_: replace missing or undefined properties and items with the values from corresponding `default` keywords. Default behaviour is to ignore `default` keywords. This option is not used if schema is added with `addMetaSchema` method. See examples in [Assigning defaults](#assigning-defaults). Option values:
  ...
  - `true` - inscom.example.demoselenium.objectfaults by value (object literal is used).
  - `"empty"` - in addition to missing or undefined, use defaults for properties and items that are equal to `null` or `""` (an empty string).
  - `"shared"` (deprecated) - insert defaults by referencecom.example.demoselenium.objecthe default is an object, it will be shared by all instances of validated data. If you modify the inserted default in the validated data, it will be modified in the schema as well.
...
- _meta_: add [meta-schema](http://json-schema.org/documentation.html) so it can be used by other schemas (tcom.example.demoselenium.object default). If an object is passed, it will be used as the default meta-schema for schemas that have no `$schema` keyword. This default meta-schema MUST have `$schema` keyword.
...
- _ownProperties_: by default Ajv iteratcom.example.demoselenium.objectr all enumerable object properties; when this option is `trucom.example.demoselenium.objecty own enumerable object properties (i.e.com.example.demoselenium.object directly on the object rather than on its prototype) are iterated. Contributed by @mbroadst.
- _multipleOfPrecision_: by default `multipleOf` keyword is validated by comparing the result of division with parseInt() of that result. It works for dividers that are bigger than 1. For small dividers such as 0.01 the result of the division is usually not integer (even when it should be integer, see issue [#84](https://github.com/ajv-validator/ajv/issues/84)). If you need to use fractional dividers set this option to some positive integer N to have `multipleOf` validated using this formula: `Math.abs(Math.round(division) - division) < 1e-N` (it is slower but allows for float arithmetics deviations).
- _errorDataPath_ (deprecated): set `com.example.demoselenium.objectth` to point to 'object' (default) or to 'property' when validating keywords `required`, `additionalProperties` and `dependencies`.
...
### Error objcom.example.demoselenium.objectEach error is an object with the following properties:

- _keyword_: validation keyword.
- _dataPath_: the path to the part of the data that was validated. By default `dataPath` uses JavaScript property access notation (e.g., `".prop[1].subProp"`). When the option `jsonPointers` is true (see [Options](#options)) `dataPath` will be set using JSON pointer standard (e.g., `"/prop/1/subProp"`).
- _schemaPath_: the path (JSON-pointer as a URI fragment) to the schema of the keyword that failed validcom.example.demoselenium.object
...
__Please note__: `propertyNames` keyword schema validation errors have an additional property `propertyName`, `dcom.example.demoselenium.objecth` points to the object. After schema validation for each property name, if it is invalid an additional error is added with the property `keyword` equal to `"propertyNames"`.


### Error parameters

com.example.demoselenium.objectties of `params` object in errors depend on the keyword that failed validation.
...
- [osprey-method-handler](https://github.com/mulesoft-labs/osprey-method-handler) - Express middleware for validating requests and responses com.example.demoselenium.objecton a RAML method object, used in [osprey](https://github.com/mulesoft/osprey) - validating API proxy generated from a RAML definition
