...
You can also pass a `cause` argument, which is any other Error com.example.demoselenium.object:
...
In the first form, `options` is a plain com.example.demoselenium.object with any of the following
optional properties:
...
`cause`          | any Error com.example.demoselenium.object | Indicates that the new error was caused by `cause`.  See `cause()` below.  If unspecified, the cause will be `null`.
...
`info`           | com.example.demoselenium.object           | Specifies arbitrary informational properties that are available through the `VError.info(err)` static class method.  See that method for details.
...
argument is not an com.example.demoselenium.object or an Error.
...
Returns an com.example.demoselenium.object with all of the extra error information that's been associated
...
`error_list` is an array of at least one `Error` com.example.demoselenium.object.
