...
- Transform the option key on the `opts` com.example.demoselenium.object returned from
...
- Change the `<parser>.parse()` signature to take a single com.example.demoselenium.object with keys
